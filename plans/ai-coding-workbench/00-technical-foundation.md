...
- [ ] 确认后端模块、测试、fixture 和未来前端目录。
- [ ] 明确 Windows 首发还是 Windows/Linux 同步支持。
- [ ] 明确自有数据目录的 `platformdirs` 规则。
- [ ] 记录现有 workspace 非 Git 根目录这一事实。

交付：`docs/adr/0001-ai-workbench-placement.md`。

### P0-02：工具能力探测

- [ ] 定义 `ToolCapabilities` 数据结构。
- [ ] 探测 Codex/Claude 可执行文件、版本和 help 能力，不依赖本地语言输出。
- [ ] Codex 探测 App Server、`exec --json`、resume。
- [ ] Claude 探测 `stream-json`、resume、fork、input stream。
- [ ] 缺失工具时返回可解释状态，不使服务启动失败。

测试矩阵：两者存在、只存在一个、都不存在、命令超时、help 输出变化。

### P0-03：脱敏 fixture 集

- [ ] 从 Codex 和 Claude 本机会话中选择覆盖主要事件形态的最小样本。
- [ ] 复制到测试 fixture 前移除 prompt 私密内容、绝对用户名、token、URL 凭据和仓库私密信息。
- [ ] 加入截断尾行、未知事件、子代理、归档、工具结果和 usage 样本。
- [ ] 记录 fixture 来源 CLI 版本和脱敏方法。

禁止把真实完整 transcript 提交为 fixture。

### P0-04：统一事件契约 v1

- [ ] 定义 event envelope、sequence、source provenance 和 data quality。
- [ ] 定义 user/assistant/tool/command/file/usage/error/unknown 事件。
- [ ] 规定 raw event 的保留和脱敏边界。
- [ ] 生成 golden normalized events。
...
- [ ] 验证 `thread/read(includeTurns=false)`，只读取一个已存在会话。
- [ ] 验证客户端正常关闭、异常退出和超时。
- [ ] 保存当前版本生成的 JSON Schema 到测试临时目录，确认版本绑定方式。
- [ ] 记录稳定 API 与 experimental API 的使用清单。

不得调用 `thread/start` 或 `turn/start`。

### P0-06：Claude stream-json 模拟验证

- [ ] 用 fake process 输出 init、partial、assistant、tool、result 和 error JSONL。
- [ ] 验证 stdout 增量解析与 stderr 分离。
- [ ] 验证半行、无效 JSON、超长行和进程提前退出。
- [ ] 研究但不执行真实 prompt 的 argv/stdin 组合。

### P0-07：CC Switch schema 兼容探测

- [ ] 保存 v10 schema 元数据 fixture，不复制用户统计明细。
- [ ] 从当前 v16 源码生成 schema 能力清单。
- [ ] 实现只读 `sqlite_master/table_info/user_version` 探测原型。
- [ ] 验证数据库不存在、忙、只读、损坏、旧列缺失和未来版本。
- [ ] 确认连接器从不打开写事务或改变 PRAGMA。

### P0-08：进程监管原型

- [ ] 用 fake CLI 验证 spawn argv、stdin、stdout、stderr、取消和超时。
- [ ] Windows 验证 process group 与 Job Object 方案。
- [ ] 定义退出状态、interrupted 和 orphan cleanup。
- [ ] 验证背压和有限内存缓冲。

### P0-09：Spike 决策归档

- [ ] 将验证事实更新到架构和 project context。
- [ ] 更新 Phase 1–4 的接口假设。
- [ ] 记录未解决问题和放弃的方案。
...
实际路径由 P0-01 确认后替换；计划批准前不得把占位命令当作已存在命令执行。
