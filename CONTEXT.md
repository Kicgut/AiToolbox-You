...
- **项目登记项（Project Registration）**：用户确认的项目规范化根目录与显示名。`.git` 只能帮助发现候选目录，不是登记、统计或归属的必要条件。
...
- **项目级模型用量**：按 Workbench 已确认项目归属聚合的同一套模型指标。自动归属选择原生/受控 cwd 所在的最长已登记项目根目录；未匹配保持未归属。近期 CC Switch 明细仅在 `session_id` 精确匹配“工具 + 原生会话 ID”时继承该归属；全历史基线和缺口由该项目原生 JSONL 建立或增量回退。
