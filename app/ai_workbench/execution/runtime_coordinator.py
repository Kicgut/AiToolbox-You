...
from app.ai_workbench.event_persistence import persist_event, persist_status_change
from app.ai_workbench.execution.claude_runtime import ClaudeAdapter
from app.ai_workbench.execution.codex_runtime import AppServerClient, BusinessError, CodexExecClient, execute_with_fallback
from app.ai_workbench.execution.supervisor import reconcile_stale_runs
from app.ai_workbench.execution.supervisor import terminate_process_tree
...
class RuntimeCoordinator:
    ...
    def __init__(self, db_path: Path, *, executor: Callable[[dict[str, Any], dict[str, Any]], Any] | None = None) -> None:
    ...
    def execution_available(self) -> bool:
        ...
        return self._executor is not None or self.real_execution_enabled
    ...
    def request_cancel(self, run_id: str) -> bool:
        ...
        for waiter in waiters:
            waiter["decision"] = "cancel"
            waiter["event"].set()
        return bool(process is not None and terminate_process_tree(process))
    ...
    def _execute(self, run_id: str) -> None:
        ...
        try:
            ...
            if run["source_native_session_id"]:
                lease = self._acquire_source_lease(dict(run))
                run = dict(run)
            ...
            if self._executor is not None:
                ...
                result = self._execute_real(dict(run), dict(step))
                events_already_persisted = True

            self._persist_result(run_id, step["id"], dict(run), result, events_already_persisted=events_already_persisted)
            ...
            if lease:
                try:
                    self._release_lease(run_id, lease)
    ...
    def _execute_real(self, run: dict[str, Any], step: dict[str, Any]) -> Any:
        ...
        if run["tool"] == "claude":
            adapter = ClaudeAdapter(cwd=run["cwd"], env=env, on_process=lambda process: self._register_process(run["id"], process, run["tool"], step.get("timeout_ms")), on_event=lambda event: self._persist_live_event(run, step, event))
            if run["mode"] == "resume":
        ...
        return execute_with_fallback(
            step["prompt_text"],
            app=AppServerClient(cwd=run["cwd"], env=env, on_process=lambda process: self._register_process(run["id"], process, run["tool"], step.get("timeout_ms")), approval_handler=lambda request: self._await_native_approval(run, step, request), approval_delivery_handler=lambda native_id, decision, delivered: self._record_native_approval_delivery(run, step, native_id, decision, delivered), on_event=lambda event: self._persist_live_event(run, step, event)),
            exec_client=CodexExecClient(cwd=run["cwd"], env=env, on_process=lambda process: self._register_process(run["id"], process, run["tool"], step.get("timeout_ms")), on_event=lambda event: self._persist_live_event(run, step, event)),
            mode=run["mode"],
        ...
        def update(target: Any) -> None:
            ...
            if row and row["state"] == "starting":
                persist_status_change(
                    ...
                    run_updates={"dispatch_state": "dispatched", "dispatch_committed_at": _now()},
                )
        if conn is not None:
            update(conn)
        else:
    ...
    def _await_native_approval(self, run: dict[str, Any], step: dict[str, Any], request: dict[str, Any]) -> dict[str, Any]:
        ...
        operation = "command" if "commandExecution" in method else "file_change"
        command = params.get("command")
        approval_event = threading.Event()
        waiter = {"run_id": run["id"], "event": approval_event, "decision": "cancel"}
        with connect_workbench_db(self.db_path) as conn:
            item = create_approval(
                ...
                risk_level="high", command_argv=[command] if command else None,
                cwd=params.get("cwd") or run["cwd"], reason=params.get("reason"),
            )
            ...
            persist_event(conn, {
                ...
                "payload": {"approval_id": item["id"], "native_request_id": native_id, "operation": operation,
                            "target_summary": item["target_summary"], "risk_level": "high", "reason": params.get("reason")},
            })
        try:
    ...
    def _persist_live_event(self, run: dict[str, Any], step: dict[str, Any], item: dict[str, Any]) -> None:
        ...
        event = dict(item)
        event.update(run_id=run["id"], step_id=step["id"], event_id=str(uuid.uuid4()))
        ...
        with connect_workbench_db(self.db_path) as conn:
            persist_event(conn, event, run_updates=updates or None)
        if run.get("source_native_session_id") and run.get("lease_generation"):
    ...
    def _release_lease(self, run_id: str, lease: tuple[str, int]) -> None:
        with connect_workbench_db(self.db_path) as conn:
            release_writer_lease(conn, physical_session_key=lease[0], run_id=run_id, lease_generation=lease[1])
    ...
    def _persist_result(self, run_id: str, step_id: str, run: dict[str, Any], result: Any, *, events_already_persisted: bool = False) -> None:
        with connect_workbench_db(self.db_path) as conn:
            ...
            if fresh["state"] == "cancel_requested":
                ...
                terminal = "failed" if timed_out and timed_out["failure_code"] == "timeout" else "cancelled"
                persist_status_change(
                    conn, run_id=run_id, step_id=step_id, state=terminal, source_tool=run["tool"],
                    reason="timeout" if terminal == "failed" else "cancelled", step_state=terminal,
                    run_updates={"finished_at": _now()},
                )
                return
            ...
            persist_status_change(
                ...
                step_state=terminal,
                run_updates={"finished_at": _now(), "execution_path": result.execution_path, "failure_message": result.stderr or None},
            )
    ...
    def _fail(self, run_id: str, step_id: str | None, tool: str | None, code: str, message: str) -> None:
        ...
        with connect_workbench_db(self.db_path) as conn:
            ...
            if row is None or row["state"] in {"succeeded", "failed", "cancelled", "interrupted"}:
                return
            state = "interrupted" if row["state"] == "cancel_requested" else "failed"
