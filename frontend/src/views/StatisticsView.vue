...
type Metric = { value: number | null; availability: string; quality: string; source: string; reason_code?: string | null }
...
async function load() {
  ...
  try {
    ...
    if ([overviewResponse, seriesResponse, connectorResponse, breakdownResponse, reliabilityResponse, qualityResponse, conflictsResponse, auditResponse].some(response => !response.ok)) throw new Error('统计服务暂时不可用')
    ...
    <template v-else-if="overview">
      ...
      <section class="metric-grid" aria-label="统计指标">
        <article v-for="(item, key) in overview.metrics" :key="key" class="metric-card">
          <div class="metric-header"><h3>{{ key }}</h3><span :class="['quality-badge', status(item)]">{{ status(item) }}</span></div>
      ...
      <section class="statistics-section" aria-labelledby="trend-heading">
        ...
        <div class="table-wrap"><table><thead><tr><th>日期</th><th>请求</th><th>输入 token</th><th>输出 token</th><th>质量</th></tr></thead><tbody>
          <tr v-for="point in series" :key="point.bucket_start_utc"><td>{{ point.bucket_date }}</td><td>{{ display(point.metrics.request_count) }}</td><td>{{ display(point.metrics.input_tokens) }}</td><td>{{ display(point.metrics.output_tokens) }}</td><td><span :class="['quality-badge', status(point.metrics.input_tokens)]">{{ status(point.metrics.input_tokens) }}</span></td></tr>
          <tr v-if="!series.length"><td colspan="5" class="empty">当前筛选范围没有数据</td></tr>
        </tbody></table></div>
      </section>

      <section class="statistics-columns">
        <section class="statistics-section" aria-labelledby="breakdown-heading"><div class="section-heading"><h3 id="breakdown-heading">按工具拆分</h3></div><div class="table-wrap"><table><thead><tr><th>工具</th><th>模型</th><th>请求</th><th>输入</th></tr></thead><tbody><tr v-for="row in breakdown" :key="`${row.tool}-${row.model}-${row.provider}`"><td>{{ row.tool ?? '—' }}</td><td>{{ row.model ?? '—' }}</td><td>{{ display(row.metrics.request_count) }}</td><td>{{ display(row.metrics.input_tokens) }}</td></tr><tr v-if="!breakdown.length"><td colspan="4" class="empty">暂无拆分数据</td></tr></tbody></table></div></section>
