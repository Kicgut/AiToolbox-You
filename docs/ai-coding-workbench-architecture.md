...
| 项目登记项 | Workbench | 用户确认的规范化根目录及类型：可接收会话的“项目”，或只组织子项目的“工作区容器”；`.git` 只可作为发现提示。 |
...
| 项目与归属 | 项目 UUID、用户确认的规范化根目录、显示名、类型（`project` / `workspace`）；会话主项目、归属来源（`native_cwd` / `controlled_run` / `manual`）、确认时间与可选非敏感说明。 |
...
项目首先必须被用户登记为一个规范化根目录，并明确选择两种类型之一：`project` 是可接收会话和项目级统计的实际项目；`workspace` 是容纳多个项目的工作区容器，只组织、汇总和显示待归属用量，不能成为会话主项目。`.git` 可以帮助建议候选根目录或类型，但不是必要条件，也不能单独决定归属。

自动归属仅检查原生会话中明确记录的 cwd 或 Workbench 受控运行的 cwd：在所有已登记的 `project` 根目录中，选择**最长匹配根目录**作为主项目；没有项目匹配、缺少 cwd 或 cwd 仅为工作区盘符时进入“未归属用量”。若 cwd 位于一个 `workspace` 容器内，则额外标记该容器，便于在容器视图集中处理，但不得自动分配给该容器或其任一子项目。

这条规则的例子如下：

| 原生 / 受控 cwd | 已登记根目录 | 自动结果 |
|---|---|---|
...
| `E:\project\项目A\src` | `E:\project`（工作区容器）、`E:\project\项目A`（项目） | 归属 `项目A`。 |
| `E:\project\项目D` | `E:\project`（工作区容器），未登记项目 D | 未归属，但标记“位于 `E:\project` 工作区”。 |
| `E:\` | 上述任一根目录 | 未归属；不得因提示词或后续命令猜测属于其中之一。 |

嵌套根目录默认不自动登记。若用户明确把 `E:\statistics-toolbox-You\docs\diagrams` 另登记为独立 `project`，最长匹配规则会优先归属该子项目，并在登记时提示它会从父项目统计中分离；用户需明确确认。相反，`E:\project` 下有项目 A、B、C 时，应将 `E:\project` 登记为 `workspace`，再分别登记 A、B、C；容器页可汇总子项目和“容器内未归属”，但不会把它们误算成一个父项目。多仓库、单体仓库和没有 Git 的目录均适用这一规则。
...
| 2026-08-03 | 项目登记区分 `project` 与 `workspace`：容器不接收会话主归属；自动归属只在实际项目根目录中做最长路径匹配，并将容器内未登记子目录保留为未归属。 | 已确认 |
