...
项目归属由 Workbench 轻量会话索引决定：已发现的“工具 + 原生会话 ID”绑定一个项目目录；CC Switch 最近明细只在 `session_id` 与该索引精确匹配时才归入项目。未匹配、缺少 ID 或来源语义不明的行进入“未归属用量”，可在全局页显示，但不得自动分配给项目。

首次把项目登记到 Workbench 时，为了获得完整历史，解析该项目原生 JSONL 并建立按天、工具、provider、模型的轻量 `project_usage_daily` 基线。随后优先以 CC Switch 仍保留的最近明细窗口更新这些日汇总；同一项目/日期/维度采用一个已标明来源的结果，CC Switch 成功覆盖时替换对应原生基线，不得两份相加。CC Switch 未覆盖、不可归属或不兼容时，只增量解析受影响的原生源文件作为回退。
